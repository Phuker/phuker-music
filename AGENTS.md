...
- **`player.generate()` 的 `dir_path` 不能以 `/` 或 `\` 结尾** — `player.py:123` 有 `_assert` 校验；但 CLI（`cli.py:19`）和 albums 配置（`albums.py:47`）会先经 `os.path.abspath()` 自动去掉尾部斜杠，所以正常使用不会触发
- **`_utils._assert()` 抛出 `AssertionError`**，不是 `ValueError` — 如果捕获异常，注意类型
- **`-v` / `--verbose` 启用调试日志** — 顶层和子命令级 `-v` 叠加，`>=1` 即设 `DEBUG` 级别（`cli.py:87`）
